...
void read_class(class_info* pClassInfo, const char* pFilename) {
    ...
    readU16(pClassInfo->minorVersion);
    readU16(pClassInfo->majorVersion);

    readU16(pClassInfo->constantPoolCount);
    ...
    readU16(pClassInfo->accessFlags);
    readU16(pClassInfo->thisClass);
    readU16(pClassInfo->superClass);
    ...
    for (uint16_t i = 0; i < pClassInfo->methodsCount; ++i) {
        read_method(pClassInfo->pMethods + i, pClassFile);
    } 

    readU16(pClassInfo->attributesCount);
    pClassInfo->attributes = malloc(sizeof(attr_info**) * pClassInfo->attributesCount);
    for (uint16_t i = 0; i < pClassInfo->attributesCount; ++i) {
        pClassInfo->attributes[i] = read_attr(pClassFile);
    }
}